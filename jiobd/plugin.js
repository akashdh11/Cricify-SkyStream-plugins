(function() {
    ...
    async function fetchM3U() {
        const url = `${manifest.baseUrl}/JIOBDNEW.php`;
        const headers = {
        ...
        if (status >= 200 && status < 300) {
            ...
            throw new Error(`HTTP Error ${status || 'No Response'} fetching JIO BD M3U`);
        }
    }
    ...
    async function loadStreams(url, cb) {
        try {
            ...
            if (channelData.drmKeys || targetUrl.includes(".mpd") || targetUrl.includes("play.php")) {
                try {
                    ...
                    } else if (response.finalUrl && response.finalUrl !== targetUrl) {
                        // Handle redirection if it leads to a direct stream
                        targetUrl = response.finalUrl;
                        if (!targetUrl.toLowerCase().includes("extension=")) {
                            if (targetUrl.includes(".m3u8")) targetUrl += targetUrl.includes("?") ? "&extension=.m3u8" : "?extension=.m3u8";
                            else if (targetUrl.includes(".mpd")) targetUrl += targetUrl.includes("?") ? "&extension=.mpd" : "?extension=.mpd";
                        }
                    }
                } catch (e) {}
